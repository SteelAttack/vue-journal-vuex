<template>
  <div class="entry-title d-flex justify-content-between p-2">
    <div>
      <span class="text-success fs-3 fw-bold">15</span>
      <span class="mx-1 fs-3">Noviembre</span>
      <span class="mx-2 fs-4 fw-light">2022, jueves</span>
    </div>
    <div>
      <button class="btn btn-danger mx-2">
        Borrar
        <i class="fa fa-trash-alt"></i>
      </button>
      <button class="btn btn-primary">
        Subir foto
        <i class="fa fa-upload"></i>
      </button>
    </div>
  </div>

  <hr />

  <div class="f-flex flex-column px-3 h-75">
    <textarea class="w-100" placeholder="¿Qué sucedió hoy?"></textarea>
  </div>
  <Fab icon="fa-floppy-disk" />

  <img
    src="https://hoycuriosidades.com/wp-content/uploads/2021/05/pinguinos-930x620.jpg"
    class="img-thumbnail"
    alt="penguins"
  />
</template>

<script setup>
import Fab from '@/modules/daybook/components/Fab.vue'
</script>

<style lang="scss" scoped>
textarea {
  font-size: 20px;
  border: none;
  height: 100%;

  &:focus {
    outline: none;
  }
}
img {
  width: 200px;
  position: fixed;
  bottom: 150px;
  right: 20px;
  box-shadow: 0px 5px 10px rgba($color: #000000, $alpha: 0.2);
}
</style>
